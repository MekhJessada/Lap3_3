public class OX {
    ...
    public boolean put(int col, int row) {
        if(col+1 > 3 || row+1 > 3){
            return false;
        }else if(!table[col+1][row+1].equals("-")){
            return false;
        }else{
            round++;
            table[col+1][row+1] = getPlayer();
            return true;
        }
    ...
    public boolean checkWin(int col, int row) {
        ...
        boolean flax4=true;
        for (int i = 0; i < 3; i++){
            if (table[i+1][row+1].equals("-")||!table[i+1][row+1].equals(player)){
                flax1=false;
            }if(table[col+1][i+1].equals("-")||!table[col+1][i+1].equals(player)){
                flax2=false;
